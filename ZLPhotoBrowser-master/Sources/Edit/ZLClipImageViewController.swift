...
class ZLClipImageViewController: UIViewController {
    ...
    static let bottomToolViewH: CGFloat = 125
    ...
    var maxClipFrame: CGRect = {
        ...
        rect.size.width = UIScreen.main.bounds.width - 16 * 2
        rect.size.height = UIScreen.main.bounds.height - insets.top - ZLClipImageViewController.bottomToolViewH
        return rect
    }()
    ...
    override func viewDidLayoutSubviews() {
        ...
        self.clipRatioColView.backgroundColor = UIColor.red
    ...
    func setupUI() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(panTest(recognizer:)))
//        self.overlayView.addGestureRecognizer(pan)
    ...
    @objc private func panTest(recognizer: UIPanGestureRecognizer) {
        let view = self.overlayView
        let translation = recognizer.translation(in: view?.superview)
        self.overlayView.center = CGPoint(x: self.overlayView.center.x + translation.x, y: self.overlayView.center.y + translation.y)
        recognizer.setTranslation(.zero, in: self.overlayView.superview)
    }
